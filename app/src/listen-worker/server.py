...
from fastapi import FastAPI, File, UploadFile, HTTPException, BackgroundTasks, Depends, Header, Request
...
PARAKEET_KEY = os.getenv("PARAKEET_KEY")
if not PARAKEET_KEY:
    print("Error: PARAKEET_KEY environment variable is not set. Application cannot start.")
    sys.exit(1)

HOST = os.getenv("HOST", "0.0.0.0")
PORT = int(os.getenv("PORT", "3011"))
PARAKEET_REQUIRE_FORWARDED_SSL = os.getenv("PARAKEET_REQUIRE_FORWARDED_SSL")
...
# SSL Forwarding Check Dependency
async def check_forwarded_ssl(request: Request, x_forwarded_ssl: Optional[str] = Header(None)):
    if PARAKEET_REQUIRE_FORWARDED_SSL == "1":
        if not x_forwarded_ssl or x_forwarded_ssl.lower() != "on":
            raise HTTPException(
                status_code=403, 
                detail="Forbidden: Missing or invalid x-forwarded-ssl header. SSL is required."
            )
...
app = FastAPI(
    ...
    docs_url="/docs",
    redoc_url="/redoc",
    dependencies=[Depends(check_forwarded_ssl)]
)
...
async def verify_api_key(authorization: Optional[str] = Header(None)):
    ...
    if scheme.lower() != 'bearer' or token != PARAKEET_KEY:
...
@app.post("/transcribe/srt", dependencies=[Depends(verify_api_key)])
