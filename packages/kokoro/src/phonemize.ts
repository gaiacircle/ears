...
/**
 * Result of splitting text on a regex while keeping delimiters
 */
interface SplitResult {
  /** Whether this segment is a match (delimiter) or not */
  match: boolean;
 ...
 * @returns The split string with match indicators.
 */
 ...
 * @param match - The matched number
 * @returns The phonetic equivalent
 */
 ...
 * @param match - The matched currency
 ...
 * @param norm - Whether to normalize the text
 * @returns The phonemized text
 */
export async function phonemize(text: string, language: "a" | "b" = "a", norm: boolean = true): Promise<string> {
  // 1. Normalize text
  if (norm) {
